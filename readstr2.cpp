...
{ // read fields from 
	...
	cout << "(Student)Enter<y> continue, or <Enter> to end: " << flush;
	is.getline(s.id, 30);
...
{ // read fields from 
	cout << "(CourseRegistration)Enter<y> continue, or <Enter> to end: " << flush;
...
{ // read fields from 
	cout << "(Course)Enter<y> continue, or <Enter> to end: " << flush;
	is.getline(s.course_id, 30);
	if (strlen(s.course_id) == 0) return is;
	cout << "Enter course id: " << flush; is.getline(s.course_id, 30);
...
void read(char filename[20]) {
	...
	while (1)
	{
		...
		else if (ch == '$')
		{
			...
			cout << "id\t" << "address\t" <<"first_enroll\t"<< "credit_hour" << endl<<endl;
		}
			
		else if (ch == '#')
		{
			cout << endl << "----Courseregistration----" << endl;
			cout << "course id\t" << "student id\t" << "course grade\t" << "credit_hour" << endl<<endl;
		}
		else if (ch == '@')
		{
			cout << endl << "----------Course-----------" << endl;
			cout << "course id\t" << "student id\t" << "course grade\t" << "credit_hour\t"
				<<"teacher\t"<<"classroom\t" << endl<<endl;
		}
		else if (ch == '%')
			cout << endl;
		else
